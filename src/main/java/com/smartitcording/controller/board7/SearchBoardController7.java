...
import com.smartitcording.service.BoardService7;

@Controller
@RequestMapping("/sboard7/*")
public class SearchBoardController7 {

	 private static final Logger logger = LoggerFactory.getLogger(SearchBoardController7.class);

	  @Inject
	  private BoardService7 service;
	...
	//  SearchCritera cri = 검색타입,키워드 속성 가짐. // xml= listsearch - pageStart, pageNum +search에 맞는 모든 리스트데이터 받음 
	    ...
	    model.addAttribute("sboardNum","7");// /sboard"2" 표기하기 위해서.. -> list.jsp ahref에 ${sboardNum} 으로 사용함.
	    /**/
	    return "/sboard/list";
	  }
	  ...
	  public String modifyPagingGET(int bno, @ModelAttribute("cri") SearchCriteria cri, Model model) throws Exception {
	    ...
	    model.addAttribute("sboardNum","7");
	  ...
	  public String like(@RequestParam("bno") int bno,@RequestParam("uid") String uid,BoardVO board) throws Exception {
	    ...
	    return "redirect:/sboard7/readPage?bno="+bno+"&uid="+uid; // 리턴이 되어도 uri 자체가 바뀌진 않는다 그래서 애초에 요청이었던 /sboard/readPage/like?bno=~~~ 이런식으로 된다. -> scm 플레이어에 의한 redirect 충돌에 의한것이었음.
