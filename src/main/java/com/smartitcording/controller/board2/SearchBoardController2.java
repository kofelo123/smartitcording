package com.smartitcording.controller.board2;
...
import com.smartitcording.service.BoardService2;

@Controller
@RequestMapping("/sboard2/*")
public class SearchBoardController2 {

	 private static final Logger logger = LoggerFactory.getLogger(SearchBoardController2.class);
	...
	//  SearchCritera cri = 검색타입,키워드 속성 가짐. // xml= listsearch - pageStart, pageNum +search에 맞는 모든 리스트데이터 받음 
	    ...
	    model.addAttribute("sboardNum","2");// /sboard"2" 표기하기 위해서.. -> list.jsp ahref에 ${sboardNum} 으로 사용함.
	    /**/
	    return "/sboard/list";
	  }
	  ...
	  public String modifyPagingGET(int bno, @ModelAttribute("cri") SearchCriteria cri, Model model) throws Exception {
	    ...
	    model.addAttribute("sboardNum","2");
	  ...
	  public String like(@RequestParam("bno") int bno,@RequestParam("uid") String uid,BoardVO board) throws Exception {
	    ...
	    return "redirect:/sboard2/readPage?bno="+bno+"&uid="+uid; // 리턴이 되어도 uri 자체가 바뀌진 않는다 그래서 애초에 요청이었던 /sboard/readPage/like?bno=~~~ 이런식으로 된다. -> scm 플레이어에 의한 redirect 충돌에 의한것이었음.
